...
const AddEmployeePage = () => {
  ...
  const { data: Hrdata ,refetch:hrDetails } = useQuery({
  ...
  const handleAddToTeam = async (employee) => {
    ...
    if (!packageName) {
      return toast.error("Select a package");
    }
    if(packageName =='BASIC' && Hrdata?.teamMember == 5){
        return toast.error('Increase Your Employee Limits')
    }
    if(packageName == 'PREMIUM' && Hrdata?.teamMember == 10){
        return toast.error('Increase Your Employee Limits')
    }
    if(packageName == 'ENTERPRISE' && Hrdata?.teamMember == 20){
        return toast.error('Increase Your Employee Limits')
    }
    const { data } = await axiosSecure.post("/addtocompany", {
      ...employee,
      hremail,
      companyName,
      companyLogoUrl,
